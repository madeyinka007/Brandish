...
export default function SiteFooter({ title, categories }: { title: string; categories: Category[] }) {
  ...
  return (
    <footer className={`flex flex-col gap-10 bg-surface-footer py-14 text-white sm:py-20 ${PAD}`}>
      {/* Brand bar */}
      <div className="flex flex-wrap items-center justify-between gap-6 border-b border-[#2e2e2e] pb-10">
      ...
      <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_2fr]">
        {cols.map((c) => (
        ...
        <div id="newsletter" className="flex scroll-mt-24 flex-col gap-4">
      ...
      {/* Legal */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#2e2e2e] pt-6 text-[13px] text-[#a8a8a8]">
